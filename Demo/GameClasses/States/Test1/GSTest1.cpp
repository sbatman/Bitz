...
void GSTest1::OnEnter(const Bitz::GameLogic::GameState_Ptr& preceedingState)
{
...
void GSTest1::OnUpdate(double ms)
{
	...
	{
		for (const std::shared_ptr<Orb>& o : _TestOrbs)o->Update(xLimit, yLimit, static_cast<float_t>(xLimit*0.5f), static_cast<float_t>(yLimit*0.5f));
	}
}
