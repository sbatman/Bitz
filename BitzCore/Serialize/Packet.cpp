...
using namespace Bitz::Serialize;
...
void Packet::AddByteArray(uint8_t const* value, uint32_t const length)
{
	...
	Memcpy(_Data + _DataPos, _DataArraySize, &length, sizeof(uint32_t));
	_DataPos += 4;
	Memcpy(_Data + _DataPos, _DataArraySize, value, length);
	_DataPos += length;
	_ParamCount++;
}
...
void Packet::UpdateObjects()
{
	...
	{
		uint8_t type = 0;
		Memcpy(&type, sizeof(uint8_t), _Data + ArrayPos, sizeof(uint8_t));
		ArrayPos += 1;
...
{
	T * value = new T(0);
	Memcpy(value, sizeof(T), _Data + offset, sizeof(T));
	return value;
}
...
void Packet::AddToDataArray(ParamTypes type, int32_t dataAmmount, T const *  dataPosition)
{
	...
	Memcpy(_Data + _DataPos, _DataArraySize, dataPosition, dataAmmount);
